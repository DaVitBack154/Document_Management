...
export default function FromManual() {
    ...
    const init = async () => {
      try {
        let regisDoc = await axios.get(
          process.env.REACT_APP_API + '/manual-get/' + id,
    ...
    try {
      ...
      if (id) {
        result = await axios.put(
          process.env.REACT_APP_API + '/manual-update/' + id,
          value,
          { withCredentials: true }
        );
      } else {
        result = await axios.post(
          process.env.REACT_APP_API + '/manual-insert/',
          value,
          { withCredentials: true }
        );
      }
  ...
  return (
    <FormItUser>
      <div className="group-form">
        <div className="head_it">ระเบียบคู่มือ</div>
        <Form
