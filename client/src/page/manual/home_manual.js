...
export default function HomeManual() {
  ...
  const column = [
    {
      ...
      width: 50,
      render: (_, record) => (
        <NavLink to={'/frommanual/' + record.id}>
  ...
  useEffect(() => {
    const init = async () => {
      let regisdoc = await axios.get(process.env.REACT_APP_API + '/manual', {
          ...
          <div className="head-table">
            <div className="box-flex">
              <div className="text-dash">
                <div className="head-db">ระเบียบคู่มือ</div>
                <div className="text-dash">Table ˃ ระเบียบคู่มือ</div>
              </div>
            </div>

            <div className="btn-add">
              <NavLink to={'/frommanual'}>
