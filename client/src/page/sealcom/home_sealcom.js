...
export default function Homesealcom() {
  ...
  const column = [
    {
      ...
      width: 50,
      render: (_, record) => (
        <NavLink to={'/fromsealcom/' + record.id}>
  ...
  useEffect(() => {
    const init = async () => {
      let regisdoc = await axios.get(process.env.REACT_APP_API + '/sealcom', {
          ...
          <div className="head-table">
            <div className="box-flex">
              <div className="text-dash">
                <div className="head-db">การประทับตราบริษัท</div>
                <div className="text-dash">Table ˃ การประทับตราบริษัท</div>
              </div>
            </div>

            <div className="btn-add">
              <NavLink to={'/fromsealcom'}>
