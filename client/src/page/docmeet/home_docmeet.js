...
export default function HomeDocmeet() {
  ...
  const column = [
    {
      ...
      width: 50,
      render: (_, record) => (
        <NavLink to={'/fromdocmeet/' + record.id}>
    ...
    {
      title: 'เวลาที่ประชุม',
      dataIndex: 'datetime_meet',
      render: (time, record) => {
        const timemer = new Date(time);
        return (
          timemer.getHours() +
          ':' +
          timemer.getMinutes() +
  ...
  useEffect(() => {
    const init = async () => {
      let regisdoc = await axios.get(process.env.REACT_APP_API + '/docmeet', {
          ...
          <div className="head-table">
            <div className="box-flex">
              <div className="text-dash">
                <div className="head-db">เอกสารการประชุม</div>
                <div className="text-dash">Table ˃ เอกสารการประชุม</div>
              </div>
            </div>

            <div className="btn-add">
              <NavLink to={'/fromdocmeet'}>
