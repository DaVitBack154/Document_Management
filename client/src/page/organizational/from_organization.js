    ...
    const init = async () => {
      try {
        let regisDoc = await axios.get(
          process.env.REACT_APP_API + '/organizational-get/' + id,
    ...
    try {
      ...
      if (id) {
        result = await axios.put(
          process.env.REACT_APP_API + '/organizational-update/' + id,
          value,
          { withCredentials: true }
        );
      } else {
        result = await axios.post(
          process.env.REACT_APP_API + '/organizational-insert/',
          value,
          { withCredentials: true }
        );
      }
  ...
  return (
    <FormItUser>
      <div className="group-form">
        <div className="head_it">โครงสร้างองค์กร</div>
        <Form
