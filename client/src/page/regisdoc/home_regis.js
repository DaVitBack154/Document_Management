...
export default function HomeRegis() {
  const [data, setData] = useState(null);

  const topicDoc = {
    tyd1: 'ใบสำคัญแสดงการจดทะเบียน',
    tyd2: 'หนังสือรับรอง',
    tyd3: 'บัญชีรายชื่อผู้ถือหุ้น',
    tyd4: 'หนังสือบริคณห์สนธิ',
    tyd5: 'ข้อบังคับ',
    tyd6: 'สมุดทะเบียนผู้ถือหุ้น',
    tyd7: 'ใบหุ้น',
    tyd8: 'อื่น ๆ (ตรงอื่น ๆ สามารถให้ระบุประเภทเอกสารเอง)',
  };
  ...
  const column = [
    {
      ...
      width: 50,
      render: (_, record) => (
        <NavLink to={'/fromregisdoc/' + record.id}>
    ...
    },
    {
      ...
      dataIndex: 'type_doc',
      render: (_, record) => {
        return <>{topicDoc[record.type_doc] ?? 'ไม่ได้ระบุ'}</>;
      },
    },
    {
      title: 'ปี',
  ...
  useEffect(() => {
    const init = async () => {
      let regisdoc = await axios.get(process.env.REACT_APP_API + '/regisdoc', {
          ...
          <div className="head-table">
            <div className="box-flex">
              <div className="text-dash">
                <div className="head-db">เอกสารทางทะเบียน</div>
                <div className="text-dash">Table ˃ เอกสารทางทะเบียน</div>
              </div>
            </div>

            <div className="btn-add">
              <NavLink to={'/fromregisdoc'}>
