...
export default function FromExternalUpdate() {
    ...
    useEffect(() => {
        const init = async () => {
            try {
                let Internalmemo = await axios.get(process.env.REACT_APP_API + '/external-get/' + id, { withCredentials: true })
                if (Internalmemo.data.status) {
                    let data = Internalmemo.data.data
                    data.date_doc = data.date_doc ? moment(data.date_doc) : moment()
                    form.setFieldsValue(data)
                    setFileName(data?.img_external)
                }
    ...
    const onFinish = async (value) => {
        ...
        value.img_external = fileName

        try {

            let result = await axios.put(process.env.REACT_APP_API + '/external-update/' + id, value, { withCredentials: true })
        ...
        <FormItUser>
            <div className='group-form'>
                <div className='head_it'>
                    EXTERNAL{/* INTERNAL MEMO - {id.type_data} */}
                </div>
