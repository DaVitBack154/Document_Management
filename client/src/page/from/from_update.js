...
export default function FromUpdate() {
    ...
    useEffect(() => {
        const init = async () => {
            try {
                let Internalmemo = await axios.get(process.env.REACT_APP_API + '/internalmemo-get/' + id, { withCredentials: true })
                if (Internalmemo.data.status) {
                    let data = Internalmemo.data.data
                    data.date_doc = data.date_doc ? moment(data.date_doc) : moment()
                    form.setFieldsValue(data)
                    setFileName(data?.img_internal_memo)
                }
    ...
    const onFinish = async (value) => {
        ...
        value.img_internal_memo = fileName

        try {

            let result = await axios.put(process.env.REACT_APP_API + '/internalmemo-update/' + id, value, { withCredentials: true })
        ...
        <FormItUser>
            <div className='group-form'>
                <div className='head_it'>
                    INTERNAL MEMO{/* INTERNAL MEMO - {id.type_data} */}
                </div>
                <Form className="it-form-wrapper" form={form} onFinish={onFinish} layout="inline" size="large">
                    ...
                    <Form.Item
                        ...
                        name={'issued'}
                    >
                        <Input placeholder="กรุณากรอกข้อมูล" value='fdfdfd' />
                    </Form.Item>
