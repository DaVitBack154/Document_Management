...
export default function Homedocpower() {
  ...
  const column = [
    {
      ...
      width: 50,
      render: (_, record) => (
        <NavLink to={'/fromdocpower/' + record.id}>
  ...
  useEffect(() => {
    const init = async () => {
      let regisdoc = await axios.get(process.env.REACT_APP_API + '/docpower', {
          ...
          <div className="head-table">
            <div className="box-flex">
              <div className="text-dash">
                <div className="head-db">คู่มือมอบอำนาจอนุมัติ</div>
                <div className="text-dash">Table ˃ คู่มือมอบอำนาจอนุมัติ</div>
              </div>
            </div>

            <div className="btn-add">
              <NavLink to={'/fromdocpower'}>
