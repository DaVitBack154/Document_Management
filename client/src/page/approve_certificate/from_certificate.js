...
export default function FromCertificate() {
    ...
    useEffect(() => {
        const init = async () => {
            try {
                let regisDoc = await axios.get(process.env.REACT_APP_API + '/certificate-get/' + id, { withCredentials: true })
    ...
    const onFinish = async (value) => {
        ...
        try {
            let result = null
            if (id) {
                result = await axios.put(process.env.REACT_APP_API + '/certificate-update/' + id, value, { withCredentials: true })
            } else {
                result = await axios.post(process.env.REACT_APP_API + '/certificate-insert/', value, { withCredentials: true })
            }
                ...
                <Form className="it-form-wrapper" form={form} onFinish={onFinish} layout="inline" size="large">
                    ...
                    >
                        <Select className="" placeholder="กรุณาเลือก">
                            <Select.Option value={'tyd1'}>ใบสำคัญแสดงการจดทะเบียน</Select.Option>
                            <Select.Option value={'tyd2'}>หนังสือรับรอง</Select.Option>
                            <Select.Option value={'tyd3'}>บัญชีรายชื่อผู้ถือหุ้น</Select.Option>
                            <Select.Option value={'tyd4'}>หนังสือบริคณห์สนธิ</Select.Option>
                            <Select.Option value={'tyd5'}>ข้อบังคับ</Select.Option>
                            <Select.Option value={'tyd6'}>สมุดทะเบียนผู้ถือหุ้น</Select.Option>
                            <Select.Option value={'tyd7'}>ใบหุ้น</Select.Option>
                            <Select.Option value={'tyd8'}>อื่น ๆ (ตรงอื่น ๆ สามารถให้ระบุประเภทเอกสารเอง)</Select.Option>
                        </Select>
                    </Form.Item> */}
