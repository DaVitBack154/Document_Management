...
export default function HomeCertificate() {
    ...
    const column = [
        {
            ...
            width: 50,
            render: (_, record) => (
                <NavLink to={'/fromcertificate/' + record.id}>
    ...
    useEffect(() => {
        const init = async () => {
            let regisdoc = await axios.get(process.env.REACT_APP_API + '/certificate', { withCredentials: true })
