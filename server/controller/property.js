const property = require('../model/property.model');

module.exports.Getproperty = async (req, res) => {
  let data = await property.Getproperty();
  if (data.length < 0) {
    return res.json({ status: false, message: 'Not found information' });
  }
  return res.json({ status: true, data: data });
};

module.exports.Createproperty = async (req, res) => {
  let body = req.body;
  let regisdata = await property.Createproperty(body);
  if (regisdata == false)
    return res.json({ status: false, message: 'failed to insert' });
  return res.json({ status: true, message: 'successfully', data: regisdata });
};

module.exports.UpdatepropertyByID = async (req, res) => {
  let { id } = req.params;
  let body = req.body;
  let update = await property.UpdatepropertyByID(id, body);
  if (update == false) {
    return res.json({ status: false, message: 'Update Failed' });
  }
  return res.json({ status: true, message: 'Update Successfuly' });
};

module.exports.GetpropertyID = async (req, res) => {
  let { id } = req.params;
  let data = await property.GetpropertyID(id);
  if (data.length < 1) {
    return res.json({ status: false, message: 'Not found information' });
  }
  return res.json({ status: true, data: data[0] });
};
